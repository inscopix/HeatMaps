...
for i in range(len(stimuli_files)):
    ...
    if not data.empty:
        ...
        ax.set_xticklabels(
            data.columns[cm.dendrogram_col.reordered_ind],
            rotation=45,
            fontsize=label_fontsize,
        )
        ax.set_yticks(np.arange(data.shape[0]))
        ax.set_yticklabels(
            data.index[cm.dendrogram_row.reordered_ind], fontsize=label_fontsize
        )
...
subtitle = fig.text(
    0.5, 0.9, f"Folder: {lowest_level_folder}", fontsize=12, ha="center"
)

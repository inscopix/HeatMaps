...
# Define the range of corr_threshold values
corr_thresholds = np.linspace(0.1, 1, n_rows * n_cols)

# Loop through the subplots and corr_threshold values
for index, corr_threshold in enumerate(corr_thresholds):
    # Calculate row and column index for the current subplot
    row = index // n_cols
    col = index % n_cols

    # Create and visualize the graph with the current corr_threshold value
    ...
    visualize_graph(axes[row, col], G, node_colors)
    axes[row, col].set_title(f"Correlation Threshold: {corr_threshold:.2f}")

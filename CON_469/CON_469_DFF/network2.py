...
neuron_names = ['C00', 'C01', 'C02', 'C03', 'C04', 'C05', 'C06', 'C08', 'C09', 'C10', 'C11', 'C12', 'C13', 'C14', 'C15', 'C16', 'C18', 'C19']
...
for name in neuron_names:
    G.add_node(name, pos=(df_props.loc[name, 'CentroidX'], df_props.loc[name, 'CentroidY']))
    ...
    for j in range(len(neuron_names)):
        for k in range(j+1, len(neuron_names)):
            ...
            if abs(correlation_matrices[i].loc[neuron_names[j], neuron_names[k]]) > 0.5:
                G.add_edge(neuron_names[j], neuron_names[k], weight=correlation_matrices[i].loc[neuron_names[j], neuron_names[k]])
...
nx.draw_networkx_edges(G, pos, width=list(weights), edge_color=weights, edge_cmap=plt.cm.coolwarm)
nx.draw_networkx_labels(G, pos, {name: name for name in neuron_names}, font_color='white')
plt.axis('off')
plt.show()

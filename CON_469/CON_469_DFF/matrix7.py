...
fig, ax = plt.subplots(nrows=3, ncols=4, figsize=(20, 10), gridspec_kw={'wspace': 0.3, 'hspace': 0.4})
ax = ax.ravel()

# Draw the heatmaps with no cell labels and a centered colorbar
for i in range(len(stimuli_files)):
    if i == 0:
        sns.heatmap(correlation_matrices[i], cmap="coolwarm", vmin=-1, vmax=1, ax=ax[i], cbar_kws={'orientation': 'horizontal', 'label': 'Correlation Coefficient'}, annot=True, fmt='.2f', annot_kws={"fontsize":8})
        ax[i].set_title(stimuli_files[i][:-4], fontsize=14)
    else:
        sns.heatmap(correlation_matrices[i], cmap="coolwarm", vmin=-1, vmax=1, ax=ax[i], cbar=False, annot=True, fmt='.2f', annot_kws={"fontsize":8})
        ax[i].set_title(stimuli_files[i][:-4], fontsize=14)

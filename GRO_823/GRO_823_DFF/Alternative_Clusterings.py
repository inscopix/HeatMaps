...
from sklearn.cluster import KMeans, DBSCAN, MeanShift, AgglomerativeClustering, SpectralClustering, OPTICS
...
data = data.dropna()
data = data.sample(1000)  # Adjust the number to an appropriate size for your dataset
...
for i, (name, algorithm) in enumerate(clustering_algorithms):
    ...
    sns.scatterplot(x=data_2D[:, 0], y=data_2D[:, 1], hue=labels, palette='viridis', ax=axs[i], legend=None)
...
plt.tight_layout()
# Save the plot to a file instead of showing it directly
